...
import { BookOpen, FolderOpen, Sparkles, ArrowRight, Loader2, AlertCircle, FolderSearch } from 'lucide-react';
  ...
  return (
    <>
      {showPicker && (
        <FilePicker
          mode="open"
          onSelect={(p) => { setFilePath(p); setShowPicker(false); }}
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label className="block text-xs text-gray-500 mb-1.5">
            Path to your <code className="text-blue-400">.gnucash</code> file
          </label>
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 bg-gray-800 border border-white/10 rounded-lg px-3 py-2.5 text-sm text-gray-100 placeholder-gray-600 outline-none focus:border-blue-500 transition-colors font-mono min-w-0"
            />
            <button
              type="button"
              onClick={() => setShowPicker(true)}
              title="Browse for file"
              className="flex items-center gap-1.5 px-3 py-2.5 bg-gray-800 hover:bg-gray-700 border border-white/10 rounded-lg text-xs text-gray-400 hover:text-gray-200 transition-colors flex-shrink-0"
            >
              <FolderSearch size={14} /> Browse
            </button>
          </div>
          <p className="text-xs text-gray-600 mt-1.5">
            GnuCash backups (.log, .gnucash.YYYYMMDD…) will be stored in the same directory.
          </p>
        </div>
